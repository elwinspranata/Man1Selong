...
<?php $__env->startSection('title', 'Pengumuman PPDB — ' . ($school_setting->school_name ?: 'MAN 1 Lombok Timur')); ?>
...
<section class="py-16 bg-white">
    <div class="max-w-3xl mx-auto px-4">
        ...
        <?php if($school_setting->pengumuman_status): ?>
        <div class="ppdb-card p-6 md:p-8 mb-10">
            ...
            <form action="<?php echo e(route('ppdb.pengumuman')); ?>" method="GET" class="flex gap-3">
                <input type="text" name="nisn" value="<?php echo e(request('nisn')); ?>" class="ppdb-input flex-1" placeholder="Masukkan NISN atau No. Pendaftaran" required>
            ...
            <div class="mt-6">
                <?php if($result): ?>
                    <div class="bg-slate-50 border border-slate-200 p-6 rounded-xl text-center">
                        <div class="inline-flex items-center justify-center w-16 h-16 rounded-full mb-4 <?php echo e($result->status == 'accepted' ? 'bg-emerald-100 text-emerald-600' : ($result->status == 'rejected' ? 'bg-red-100 text-red-600' : 'bg-yellow-100 text-yellow-600')); ?>">
                        ...
                        <h3 class="text-xl font-bold text-slate-800 mb-1"><?php echo e($result->full_name); ?></h3>
                        <p class="text-sm font-medium text-slate-500 mb-4"><?php echo e($result->registration_number); ?> - NISN: <?php echo e($result->nisn); ?></p>
                        
                        <div class="inline-block px-4 py-2 rounded-lg font-bold text-sm tracking-wide <?php echo e($result->status == 'accepted' ? 'bg-emerald-500 text-white' : ($result->status == 'rejected' ? 'bg-red-500 text-white' : 'bg-yellow-500 text-white')); ?>">
                            STATUS: <?php echo e(strtoupper(collect($result->status_label)->first() ?? $result->status_label)); ?>
                ...
                <?php else: ?>
                    <div class="bg-red-50 border border-red-200 p-6 rounded-xl text-center">
                        <p class="text-red-600 font-medium">Data pendaftar dengan NISN / Nomor Pendaftaran <strong><?php echo e(request('nisn')); ?></strong> tidak ditemukan.</p>
                    </div>
                <?php endif; ?>
            </div>
            <?php endif; ?>
...
<?php echo $__env->make('ppdb.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\man1selong\resources\views/ppdb/pengumuman.blade.php ENDPATH**/ ?>