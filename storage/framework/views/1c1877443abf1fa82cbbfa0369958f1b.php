...
<?php $__env->startSection('title', 'Login Siswa — PPDB ' . ($school_setting->school_name ?: 'MAN 1 Lombok Timur')); ?>
...
<section class="min-h-[calc(100vh-4rem)] flex items-center justify-center py-16 bg-gradient-to-br from-slate-50 via-white to-primary-light/30 relative overflow-hidden">
    ...
    <div class="w-full max-w-md mx-auto px-4 animate-fade-in-up">
        <div class="ppdb-card p-8 md:p-10 shadow-xl">
            ...
            <form action="<?php echo e(route('ppdb.login.submit')); ?>" method="POST" class="space-y-5">
                <?php echo csrf_field(); ?>
                <div>
                    <label class="ppdb-label">Email / NISN</label>
                    <input type="text" name="login" value="<?php echo e(old('login')); ?>" class="ppdb-input" placeholder="Masukkan Email atau NISN" required autofocus>
                </div>
                <div>
            ...
            <a href="<?php echo e(route('ppdb.register')); ?>" class="w-full border-2 border-primary text-primary py-3.5 rounded-xl font-bold text-sm hover:bg-primary hover:text-white transition-all uppercase tracking-widest text-center block">
                Buat Akun
            </a>
        </div>
    </div>
...
<?php echo $__env->make('ppdb.layout', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\man1selong\resources\views/ppdb/login.blade.php ENDPATH**/ ?>