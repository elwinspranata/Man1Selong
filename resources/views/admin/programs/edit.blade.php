...
<div class="max-w-5xl mx-auto">
    ...
    <form Action="{{ route('admin.programs.update', $program) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        
        <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-200">
            <h3 class="text-2xl font-black text-slate-800 tracking-tight mb-8">Ubah Detail Program</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="icon" value="{{ old('icon', $program->icon) }}"
                        ...
                        placeholder="Contoh: flask-conical">
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-4">
                        @if($program->image)
                            <img src="{{ asset('storage/' . $program->image) }}" class="w-16 h-12 rounded-xl object-cover">
                        @endif
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <textarea name="excerpt" rows="2"
                        ...
                        placeholder="Penjelasan singkat program...">{{ old('excerpt', $program->excerpt) }}</textarea>
                </div>

                <div class="md:col-span-2 space-y-2">
                    <label class="text-sm font-bold text-slate-600 ml-1">Konten Lengkap</label>
                    <textarea name="content" id="editor" rows="15"
                        class="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/5 rounded-3xl outline-none transition-all font-medium">{{ old('content', $program->content) }}</textarea>
                </div>
