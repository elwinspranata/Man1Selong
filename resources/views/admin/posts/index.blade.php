...
<div class="space-y-8">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.posts.create') }}" class="px-8 py-4 bg-emerald-600 text-white font-black rounded-2xl shadow-lg hover:shadow-emerald-500/20 hover:-translate-y-1 transition-all flex items-center gap-2">
                    ...
                    <tr class="group hover:bg-slate-50/50 transition-colors">
                        <td class="px-8 py-6">
                            <div class="flex items-center gap-4">
                                ...
                                <div class="max-w-xs xl:max-w-md">
                                    <p class="font-bold text-slate-900 leading-tight mb-1 truncate">{{ $post->title }}</p>
                                    <p class="text-xs font-medium text-slate-400 truncate">{{ $post->excerpt }}</p>
                                </div>
                            </div>
                        </td>
                        <td class="px-8 py-6">
                            <span class="px-3 py-1 bg-emerald-50 text-emerald-600 rounded-lg text-xs font-bold">{{ $post->category ?: 'Umum' }}</span>
                        </td>
                        <td class="px-8 py-6">
                            <span class="text-sm font-semibold text-slate-600">{{ $post->created_at->format('d M Y') }}</span>
                        </td>
                        <td class="px-8 py-6">
                        ...
                        <td class="px-8 py-6 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.posts.edit', $post) }}" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <form action="{{ route('admin.posts.destroy', $post) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus berita ini?')">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="5" class="px-8 py-20 text-center">
                            <div class="flex flex-col items-center gap-4">
                                ...
                                <a href="{{ route('admin.posts.create') }}" class="text-emerald-600 font-black text-sm hover:underline">Tulis Sekarang</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
