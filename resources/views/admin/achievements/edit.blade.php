...
<div class="max-w-4xl mx-auto">
    ...
    <form Action="{{ route('admin.achievements.update', $achievement) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-200">
            <h3 class="text-2xl font-black text-slate-800 tracking-tight mb-8">Ubah Detail Pencapaian</h3>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="rank" value="{{ old('rank', $achievement->rank) }}"
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="year" value="{{ old('year', $achievement->year) }}" required
                        class="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/5 rounded-2xl outline-none transition-all font-medium"
                        placeholder="2024">
                </div>
                ...
                <div class="md:col-span-2 space-y-2">
                    ...
                    <textarea name="description" rows="4"
                        ...
                        placeholder="Ceritakan sedikit tentang pencapaian ini...">{{ old('description', $achievement->description) }}</textarea>
                </div>

                <div class="md:col-span-2 space-y-4 pt-4">
                    <label class="text-sm font-bold text-slate-600 ml-1">Ubah Ikon / Foto</label>
                    <div class="flex items-center gap-6">
                        <div class="w-20 h-20 rounded-2xl bg-slate-100 border-2 border-dashed border-slate-300 flex items-center justify-center overflow-hidden">
                            @if($achievement->icon)
                                <img src="{{ asset('storage/' . $achievement->icon) }}" class="w-12 h-12 object-contain">
                            @else
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-xs text-slate-400 mt-2 font-medium">Kosongkan jika tidak ingin mengubah.</p>
                        </div>
                    </div>
                </div>
            </div>
