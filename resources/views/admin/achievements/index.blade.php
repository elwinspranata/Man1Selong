...
@section('page_title', 'Manajemen Prestasi')
...
<div class="space-y-8">
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('admin.achievements.create') }}" class="px-8 py-4 bg-emerald-600 text-white font-black rounded-2xl shadow-lg hover:shadow-emerald-500/20 hover:-translate-y-1 transition-all flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-[2.5rem] p-8 shadow-sm border border-slate-200 group hover:border-emerald-500 hover:shadow-xl hover:shadow-emerald-500/5 transition-all duration-500 h-full flex flex-col">
            <div class="flex items-start justify-between mb-6">
                <div class="w-16 h-16 rounded-2xl bg-emerald-50 flex items-center justify-center text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-all duration-500">
                    @if($achievement->icon)
                        <img src="{{ asset('storage/' . $achievement->icon) }}" class="w-10 h-10 object-contain">
                    @else
                ...
                <div class="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    <a href="{{ route('admin.achievements.edit', $achievement) }}" class="p-2 bg-slate-50 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-xl transition-all">
                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                        </svg>
                    </a>
                    <form action="{{ route('admin.achievements.destroy', $achievement) }}" method="POST" onsubmit="return confirm('Hapus data prestasi ini?')">
                        @csrf
                        @method('DELETE')
            ...
            <div class="mb-4">
                <p class="text-[0.65rem] font-black text-emerald-600 uppercase tracking-widest mb-1">{{ $achievement->rank ?: 'Juara' }} - {{ $achievement->year }}</p>
                <h4 class="text-xl font-black text-slate-900 leading-tight">{{ $achievement->title }}</h4>
            </div>
            
            <p class="text-slate-500 font-medium text-sm leading-relaxed flex-grow">
                {{ \Illuminate\Support\Str::limit($achievement->description, 100) }}
            </p>
        </div>
        @empty
        <div class="md:col-span-2 xl:col-span-3 py-20 text-center bg-white rounded-[2.5rem] border border-dashed border-slate-200">
            <div class="flex flex-col items-center gap-4">
                ...
                <a href="{{ route('admin.achievements.create') }}" class="text-emerald-600 font-black text-sm hover:underline">Tambah Sekarang</a>
            </div>
        </div>
        @endforelse
    </div>
