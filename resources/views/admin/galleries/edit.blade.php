...
<div class="max-w-4xl mx-auto">
    ...
    <form Action="{{ route('admin.galleries.update', $gallery) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        
        <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-200">
            <h3 class="text-2xl font-black text-slate-800 tracking-tight mb-8">Ubah Detail Media</h3>
            ...
            <div class="grid grid-cols-1 gap-8">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <label class="relative flex px-6 py-5 border-2 rounded-2xl cursor-pointer transition-all peer-checked:border-emerald-500 peer-checked:bg-emerald-50/50 border-slate-100 bg-slate-50 hover:border-slate-200 focus-within:border-emerald-500" onclick="toggleMediaType('image')">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div>
                                    <p class="font-black text-slate-800">Foto</p>
                        ...
                        <label class="relative flex px-6 py-5 border-2 rounded-2xl cursor-pointer transition-all peer-checked:border-emerald-500 peer-checked:bg-emerald-50/50 border-slate-100 bg-slate-50 hover:border-slate-200 focus-within:border-emerald-500" onclick="toggleMediaType('video_url')">
                            <input type="radio" name="type" value="video_url" class="peer sr-only" {{ old('type', $gallery->type) == 'video_url' ? 'checked' : '' }}>
                            <div class="flex items-center gap-4">
                ...
                <div id="image_input_container" class="space-y-4 {{ old('type', $gallery->type) === 'image' ? 'block' : 'hidden' }}">
                    <label class="text-sm font-bold text-slate-600 ml-1">Ubah Foto (Opsional)</label>
                    <div class="flex items-center gap-4">
                        @if($gallery->type === 'image' && $gallery->image)
                            <img src="{{ asset('storage/' . $gallery->image) }}" class="w-20 h-16 object-cover rounded-xl shadow-sm">
                        @endif
                ...
                <div id="video_input_container" class="space-y-2 {{ old('type', $gallery->type) === 'video_url' ? 'block' : 'hidden' }}">
                    <label class="text-sm font-bold text-slate-600 ml-1">Tautan URL Video (Youtube/Lainnya)</label>
                    <input type="url" name="image" id="video_url" value="{{ old('type', $gallery->type) === 'video_url' ? old('image', $gallery->image) : '' }}"
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" rows="3"
                        ...
                        placeholder="Deskripsi untuk media ini...">{{ old('description', $gallery->description) }}</textarea>
                </div>
...
<script>
    ...
    // Initialize state
    toggleMediaType('{{ old("type", $gallery->type) }}');
</script>
@endsection
