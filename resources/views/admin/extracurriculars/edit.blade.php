...
<div class="max-w-5xl mx-auto">
    <div class="mb-8">
        <a href="{{ route('admin.extracurriculars.index') }}" class="inline-flex items-center gap-2 text-slate-400 hover:text-emerald-600 font-bold text-sm transition-all group">
    ...
    <form Action="{{ route('admin.extracurriculars.update', $extracurricular) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        @csrf
        @method('PUT')
        
        <div class="bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-200">
            <h3 class="text-2xl font-black text-slate-800 tracking-tight mb-8">Ubah Detail Ekstrakurikuler</h3>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="md:col-span-2 space-y-2">
                    <label class="text-sm font-bold text-slate-600 ml-1">Nama Ekstrakurikuler</label>
                    <input type="text" name="title" value="{{ old('title', $extracurricular->title) }}" required
                ...
                <div class="space-y-2">
                    <label class="text-sm font-bold text-slate-600 ml-1">Jadwal Latihan</label>
                    <input type="text" name="schedule" value="{{ old('schedule', $extracurricular->schedule) }}"
                        ...
                        placeholder="Contoh: Jumat, 15:30 - 17:00">
                </div>

                <div class="space-y-2">
                    <label class="text-sm font-bold text-slate-600 ml-1">Tempat Latihan</label>
                    <input type="text" name="location" value="{{ old('location', $extracurricular->location) }}"
                        class="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/5 rounded-2xl outline-none transition-all font-medium"
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="icon" value="{{ old('icon', $extracurricular->icon) }}"
                        class="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/5 rounded-2xl outline-none transition-all font-medium"
                        placeholder="Contoh: tent">
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center gap-4">
                        @if($extracurricular->image)
                            <img src="{{ asset('storage/' . $extracurricular->image) }}" class="w-16 h-12 rounded-xl object-cover">
                        @endif
                ...
                <div class="md:col-span-2 space-y-2">
                    <label class="text-sm font-bold text-slate-600 ml-1">Ringkasan (Excerpt)</label>
                    <textarea name="excerpt" rows="2"
                        ...
                        placeholder="Penjelasan singkat ekskul...">{{ old('excerpt', $extracurricular->excerpt) }}</textarea>
                </div>

                <div class="md:col-span-2 space-y-2">
                    <label class="text-sm font-bold text-slate-600 ml-1">Konten Lengkap</label>
                    <textarea name="content" id="editor" rows="15"
                        class="w-full px-6 py-4 bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/5 rounded-3xl outline-none transition-all font-medium">{{ old('content', $extracurricular->content) }}</textarea>
                </div>
                ...
                <div class="md:col-span-2 pt-4">
                    <label class="flex items-center group cursor-pointer">
                        ...
                        <span class="ml-3 text-sm font-bold text-slate-600 group-hover:text-slate-900 transition-colors">Aktifkan Ekstrakurikuler</span>
                    </label>
                </div>
            </div>
        </div>
