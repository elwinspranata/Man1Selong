...
@section('title', 'Detail Pendaftar - ' . $registrant->full_name)
@section('page_title', 'Detail Pendaftar')

@section('content')
<div class="max-w-5xl mx-auto space-y-8">
    <div class="flex items-center justify-between">
        <a href="{{ route('admin.ppdb.registrants.index') }}" class="inline-flex items-center gap-2 text-slate-400 hover:text-emerald-600 font-bold text-sm transition-all group">
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 overflow-hidden divide-y divide-slate-100 print:shadow-none print:border-none">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-center gap-6">
                    <div class="w-24 h-24 rounded-[2rem] bg-emerald-600 flex items-center justify-center text-white shadow-lg shadow-emerald-500/20 overflow-hidden">
                        @if($registrant->photo)
                            <img src="{{ asset('storage/' . $registrant->photo) }}" alt="Foto" class="w-full h-full object-cover">
                        @else
                            <span class="text-3xl font-black">{{ strtoupper(substr($registrant->full_name, 0, 1)) }}</span>
                        @endif
                    </div>
                    <div>
                        <div class="flex items-center gap-3 mb-1">
                            <span class="px-3 py-1 bg-slate-200 text-slate-700 rounded-lg text-xs font-black tracking-tight">{{ $registrant->registration_number }}</span>
                            <span class="px-3 py-1 bg-{{ $registrant->status_color }}-100 text-{{ $registrant->status_color }}-700 rounded-lg text-xs font-black tracking-tight uppercase">{{ collect($registrant->status_label)->first() ?? $registrant->status_label }}</span>
                        </div>
                        <h3 class="text-3xl font-black text-slate-900 tracking-tight">{{ $registrant->full_name }}</h3>
                        <p class="font-bold text-slate-500 mt-1">Jalur: <span class="capitalize">{{ $registrant->jalur }}</span> • NISN: {{ $registrant->nisn }}</p>
                    </div>
                </div>
                <div class="text-right">
                    <p class="text-xs font-bold text-slate-400 uppercase tracking-widest">Waktu Mendaftar</p>
                    <p class="font-black text-slate-800">{{ $registrant->created_at->format('d F Y, H:i') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-10 bg-yellow-50/50 print:hidden">
            ...
            <form action="{{ route('admin.ppdb.registrants.verify', $registrant) }}" method="POST" class="bg-white p-6 rounded-2xl border border-yellow-200 shadow-sm">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <select name="status" class="w-full px-4 py-3 bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/5 rounded-xl outline-none transition-all font-bold text-slate-700">
                            <option value="draft" {{ $registrant->status == 'draft' ? 'selected' : '' }}>Draft (Belum Selesai)</option>
                            <option value="pending" {{ $registrant->status == 'pending' ? 'selected' : '' }}>Menunggu Verifikasi</option>
                            <option value="verified" {{ $registrant->status == 'verified' ? 'selected' : '' }}>Terverifikasi (Lulus Berkas)</option>
                            <option value="accepted" {{ $registrant->status == 'accepted' ? 'selected' : '' }}>Diterima (Lulus Final)</option>
                            <option value="rejected" {{ $registrant->status == 'rejected' ? 'selected' : '' }}>Ditolak / Tidak Lulus</option>
                        </select>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-bold text-slate-600 ml-1">Catatan Verifikator (Opsional)</label>
                        <input type="text" name="notes" value="{{ old('notes', $registrant->notes) }}" placeholder="Contoh: Berkas KK Buram, harap upload ulang" class="w-full px-4 py-3 bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-500/5 rounded-xl outline-none transition-all font-medium">
                    </div>
                </div>
        ...
        <div class="p-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-10">
                ...
                <div class="space-y-1 pb-4 border-b border-slate-100">
                    ...
                    <p class="font-bold text-slate-700 capitalize">{{ $registrant->gender == 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
                </div>
                <div class="space-y-1 pb-4 border-b border-slate-100">
                ...
                <div class="space-y-1 pb-4 border-b border-slate-100">
                    ...
                    <p class="font-bold text-slate-700">{{ $registrant->birth_place ?? '-' }}, {{ $registrant->birth_date ? \Carbon\Carbon::parse($registrant->birth_date)->format('d-m-Y') : '-' }}</p>
                </div>
                <div class="space-y-1 pb-4 border-b border-slate-100">
                    <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Asal Sekolah</p>
                    <p class="font-bold text-slate-700">{{ $registrant->origin_school ?? '-' }} (NPSN: {{ $registrant->origin_school_npsn ?? '-' }})</p>
                </div>
                <div class="space-y-1 pb-4 border-b border-slate-100">
                    <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Email</p>
                    <p class="font-bold text-slate-700">{{ $registrant->email }}</p>
                </div>
                <div class="space-y-1 pb-4 border-b border-slate-100">
                    <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Nomor HP / WhatsApp</p>
                    <p class="font-bold text-slate-700">{{ $registrant->phone ?? '-' }}</p>
                </div>
                <div class="space-y-1 pb-4 border-b border-slate-100">
                    <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Alamat Lengkap (Prov, Kota, Kec, Desa)</p>
                    <p class="font-bold text-slate-700 leading-relaxed">{{ $registrant->address ?? '-' }}<br>
                    <span class="text-sm text-slate-500 font-medium">Kab/Kota: {{ $registrant->city ?? '-' }}, Prov: {{ $registrant->province ?? '-' }} - {{ $registrant->postal_code ?? '' }}</span>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="p-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-10">
                <div class="space-y-1 pb-4 border-b border-slate-100">
                    ...
                    <p class="text-sm text-slate-500">{{ $registrant->father_occupation ?? '-' }} (HP: {{ $registrant->father_phone ?? '-' }})</p>
                </div>
                <div class="space-y-1 pb-4 border-b border-slate-100">
                    ...
                    <p class="text-sm text-slate-500">{{ $registrant->mother_occupation ?? '-' }} (HP: {{ $registrant->mother_phone ?? '-' }})</p>
                </div>
                <div class="space-y-1 pb-4 border-b border-slate-100">
                    <p class="text-xs font-black text-slate-400 uppercase tracking-widest">Nama Wali (Opsional)</p>
                    <p class="font-bold text-slate-700">{{ $registrant->guardian_name ?? '-' }}</p>
                    <p class="text-sm text-slate-500">HP: {{ $registrant->guardian_phone ?? '-' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-10">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs font-medium text-slate-500">
                                    {{ $path ? 'Berkas Diunggah' : 'Tidak Ada Berkas' }}
                                </p>
                            </div>
                        </div>
                        @if($path)
                            <a href="{{ asset('storage/' . $path) }}" target="_blank" download class="p-2 bg-white text-emerald-600 border border-emerald-100 rounded-xl hover:bg-emerald-50 hover:border-emerald-200 transition-all font-bold text-sm shadow-sm flex items-center justify-center">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="hidden print:block text-slate-600 text-sm mt-4">
                ...
                <ul class="list-disc pl-5 mt-2">
                    @foreach($files as $label => $path)
                        <li>{{ $label }}: <strong>{{ $path ? 'Ada' : 'Kosong' }}</strong></li>
                    @endforeach
                </ul>
            </div>
